...
#------------------------------------------------------------------#
# AIX->Linux fix: $LOG is set to an absolute temp file so the      #
# redirect target is predictable regardless of cwd or env leaks    #
# (LOG=/dev/null observed in SSM/RHEL environment).  After each    #
# sqlplus block we cat $LOG so the captured output is replayed     #
# into the job log -- mirrors AIX behavior where the empty $LOG    #
# redirect silently no-op'd and output flowed to stdout.           #
#------------------------------------------------------------------#
...
#######################################################################
# AIX->Linux fix: ensure $LOG still points to our absolute temp file
# even if a sourced script clobbered it (e.g. LOG=/dev/null from env).
export LOG=/tmp/${JOBNAME}_$(basename $0 .ksh_run)_$$.sqlout
: > $LOG
...
QUIT;
CODE_BLOCK
# rj132422 - Cat-back so sp_race_to_ext output (banner, PL/SQL completed, errors)
# rj132422 - lands in the job log. Filter out the high-volume noise from
# rj132422 - dbms_output: "Post checkin date set, no need to run post checkin
# rj132422 - process." appears thousands of times (one per part), bloating the
# rj132422 - job log from ~14KB (AIX) to ~1.3MB. Drop that line; keep everything
# rj132422 - else (banner, warnings, PL/SQL completion, ORA- errors).
[ -s "$LOG" ] && grep -v 'Post checkin date set, no need to run post checkin process' "$LOG"
: > $LOG
...
#######################################################################
# Re-affirm $LOG (defensive against env leaks between steps).
export LOG=/tmp/${JOBNAME}_$(basename $0 .ksh_run)_$$.sqlout
: > $LOG
...
QUIT;
CODE_BLOCK
# rj132422 - Cat-back so sp_update_special_graphics output lands in the job log,
# rj132422 - filtering the same "Post checkin date set" noise as Step020R.
[ -s "$LOG" ] && grep -v 'Post checkin date set, no need to run post checkin process' "$LOG"
: > $LOG
...
#############################################################################
    ...
    # Cleanup the cat-back temp file from /tmp.
    rm -f "$LOG"
