...
##########################################################################
    ...
    export NTDIR=${NOVELL}altp/NAPA
    export FTPLOG=$RACE/tmp/${JOBNAME}_napa_zip_ftp.tmp

    fileput.exp $FTPFILE $NTFILE $NTDIR | tee $FTPLOG

    ntcount=$( grep 'Information ret' $FTPLOG | awk '{print $1}')
    unixcount=$(wc -c $FTPFILE | awk '{print $1}')
...
##########################################################################
    ...
    fileput.exp $FTPFILE $NTFILE $NTDIR ascii| tee $FTPLOG
