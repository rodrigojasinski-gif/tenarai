...
QUIT;
%
    # Cat-back so PKG_ULTRAMATE_BUILD dbms_output appears in the job log.
    [ -s "$LOG" ] && cat "$LOG"
    : > $LOG
...
QUIT;
%
    # Cat-back so PKG_ULTRAMATE_PREPARSE dbms_output appears in the job log.
    [ -s "$LOG" ] && cat "$LOG"
    : > $LOG
...
then
    ...
    # Final cleanup of cat-back temp file before exit.
    rm -f "$LOG"
    exit;
else
