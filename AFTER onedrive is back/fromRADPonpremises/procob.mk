...
## Modified for aix by gw 081606 to remove LINKCOUNT option. 
##                                  not used by MF Cobol Server Express 4 on aix
##Modified for aix by gw 01/19/07 to point to RACE env_precomp.mk                               
...
#
calldemo-sql:
	$(MAKE) -f $(MAKEFILE) USER=scott/wgec1Lna7qerh0 SCRIPT=calldemo sqlplus_$(RUNSQL)
sample11-sql:
	$(MAKE) -f $(MAKEFILE) USER=scott/wgec1Lna7qerh0 SCRIPT=sample11 sqlplus_$(RUNSQL)
#
...
CFLG	=  $(CFLG2) MAXOPENCURSORS=1 SQLCHECK=SEMANTICS USERID=SCOTT/wgec1Lna7qerh0 
CLEAN1	= -@rm $@.int $@.cob 
CLEAN2	= -@rm $@.o
INCL	= "/$(ACT_LVL)/race/share/inc"
...
OPT1	= -C "IBMCOMP DEFAULTCALLS=4 FOLDCOPYNAME=LOWER FOLDCALLNAME=LOWER LIST()"
...
COBFLAGS=-C IBMCOMP FOLDCOPYNAME=LOWER
...
xcgr011 xcgr012 xcgr015 xcgr016 xcgr022:
	$(PROCOB) iname=$@.cbl $(PCCFLAGS) INCLUDE=$(INCL) INCLUDE=$(STAGE_INCL) MODE=$(MODE) $(CFLG)
...
.cbl:
	$(PROCOB) iname=$*.cbl $(PCCFLAGS) INCLUDE=$(INCL) INCLUDE=$(STAGE_INCL) MODE=$(MODE) $(CFLG)
	$(COB) $(OPT1) $*.cob $(OBJ1) $(OBJS) -W e -x -L$(LIBHOME) $(COBSQLINTF) $(PROLDLIBS) 
	$(CLEAN1)
	$(CLEAN2)
