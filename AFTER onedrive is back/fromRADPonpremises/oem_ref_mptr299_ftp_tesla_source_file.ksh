...
#STEP Step020R
  ...
  then
    ########################################################################################
    # Copy the supersession file to the Network      
    ########################################################################################
    fileput.exp ${SUPER_UX} ${SUPER_NT} ${NT_DIR} | tee ${FTP_LOGFILE}
    ...
    if [ ${GOODBYTECOUNT} -eq ${FTPBYTECOUNT} ]
    then
      echo "File transfer to NT ${NT_DIR} successful. ${SUPER_UX} ftp count = ${GOODBYTECOUNT}"    
    else
      echo "File transfer of ${SUPER_UX} to NT ${NT_DIR} failed: ftp says ${FTPBYTECOUNT}, real count is ${GOODBYTECOUNT}"
      $(abndalrt.ksh 911)
    fi
  else
