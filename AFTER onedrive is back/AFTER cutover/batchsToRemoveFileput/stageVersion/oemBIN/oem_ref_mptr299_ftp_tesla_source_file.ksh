#!/bin/ksh
echo "RCS $Id: oem_ref_mptr299_ftp_tesla_source_file.ksh,v 1.9 2026/08/07 20:13:43 rj132422 Exp $"
set -xv
#*****************************************************************************************
...
#STEP Step020R
  ...
  # rj132422 - deliver the Tesla source zip to the Mitchell ftp-ssh oem_research dir (was fileput.exp to prod3nt)
  OEM_RESEARCH_DIR=${FTP_MITCHELL_BUSINESS_PATH}/${ACT_LVL}/oem_research
  if [ -e ${SOURCE_FILE} ]
  then
    # Copy the source zip to the oem_research handoff dir on the Mitchell ftp-ssh
    scp ${SOURCE_FILE} ${FTP_SFTP_USER}${FTP_SITE}:${OEM_RESEARCH_DIR}/${BACKUP_SOURCE_FILE_NAME}
    # Verify the copy is good       
    GOODBYTECOUNT="$(wc -c ${SOURCE_FILE} | awk '{print $1}')"
    FTPBYTECOUNT="$(ssh -nq ${FTP_SFTP_USER}${FTP_SITE} "wc -c ${OEM_RESEARCH_DIR}/${BACKUP_SOURCE_FILE_NAME}" | awk '{print $1}')"
    ...
    then
      ...
      MAIL_RECIP="Rpt.OEM.Activ.Tesla.Dom@Mitchell.com"
      ...
      echo "   ${FTP_SITE}:${OEM_RESEARCH_DIR}/${BACKUP_SOURCE_FILE_NAME}" >> ${MAIL_TEXT}
      echo "\nWhat to do?"                                                          >> ${MAIL_TEXT}
      echo "   1. MOVE this file into: " '\\\dept01nas\dept\PubTeams\Tesla\\'       >> ${MAIL_TEXT}
  ...
  # NOTE:
  # The Price file (PartPrices.csv) MUST be found and transferred.
  ...
  # rj132422 - mptr300 handoff dir on the Mitchell ftp-ssh (was ${NOVELL}oem on prod3nt)
  ...
  then
    ...
    ########################################################################################
    # rj132422 - scp renaming to the file name mptr300 expects (was fileput.exp to prod3nt)
    scp ${PRICE_UX} ${FTP_SFTP_USER}${FTP_SITE}:${NT_DIR}/${PRICE_NT}

    ########################################################################################
    # Verify the copy is good
    ########################################################################################
    GOODBYTECOUNT="$(wc -c ${PRICE_UX} | awk '{print $1}')"
    # rj132422 - byte count on the target via ssh (was the 'Information returned by' line from fileput.exp)
    FTPBYTECOUNT="$(ssh -nq ${FTP_SFTP_USER}${FTP_SITE} "wc -c ${NT_DIR}/${PRICE_NT}" | awk '{print $1}')"
    if [ "${GOODBYTECOUNT}" = "${FTPBYTECOUNT}" ]
    then
      echo "File transfer to ${NT_DIR} successful. ${PRICE_UX} count = ${GOODBYTECOUNT}"
    else
      echo "File transfer of ${PRICE_UX} to ${NT_DIR} failed: remote ${FTPBYTECOUNT}, real count is ${GOODBYTECOUNT}"
      $(abndalrt.ksh 911)
    fi
  else
  ...
  ##########################################################################
  # STEP NAME:  Step050R
  # STEP DESC:  Copy the Supersession file to the Network - for I/P to the reformat.
  # NOTE:
  # The Supersession file (Supersession.csv) MUST be found and transferred.
  # File name was dropped to all lowercase during unzip in prior step.
  # SUPER_NT must match the reformat job's (mptr300) INPUT_FILE_NAME.
  ##########################################################################
  ...
  then
    ...
    ########################################################################################
    # rj132422 - scp renaming to the file name mptr300 expects (was fileput.exp to prod3nt)
    scp ${SUPER_UX} ${FTP_SFTP_USER}${FTP_SITE}:${NT_DIR}/${SUPER_NT}
    ...
    # rj132422 - byte count on the target via ssh (was the 'Information returned by' line from fileput.exp)
    FTPBYTECOUNT="$(ssh -nq ${FTP_SFTP_USER}${FTP_SITE} "wc -c ${NT_DIR}/${SUPER_NT}" | awk '{print $1}')"
    if [ "${GOODBYTECOUNT}" = "${FTPBYTECOUNT}" ]
    then
      echo "File transfer to ${NT_DIR} successful. ${SUPER_UX} count = ${GOODBYTECOUNT}"
    else
      echo "File transfer of ${SUPER_UX} to ${NT_DIR} failed: remote ${FTPBYTECOUNT}, real count is ${GOODBYTECOUNT}"
      $(abndalrt.ksh 911)
    fi
  else
  ...
  # File name was dropped to all lowercase during unzip in prior step.
  # CATLG_NT must match the reformat job's (mptr300) INPUT_FILE_NAME.
  ##########################################################################
  CATLG_ZIP=${RACE}/tmp/${JOBNAME}/partscatalog_*.csv
  CATLG_UX=${RACE}/tmp/${JOBNAME}/partscatalog_combined.csv
  CATLG_NT=${JOBNAME}_raw_tesla_catlg.dat
  FTP_LOGFILE=${RACE}/tmp/${JOBNAME}_${STEPNAME}_transfer_log.tmp
  # rj132422 - mptr300 handoff dir on the Mitchell ftp-ssh (was ${NOVELL}oem on prod3nt)
  NT_DIR=${FTP_MITCHELL_BUSINESS_PATH}/${ACT_LVL}/oem/incoming
  ...
  then
    ...
    ########################################################################################
    # rj132422 - scp renaming to the file name mptr300 expects (was fileput.exp to prod3nt)
    scp ${CATLG_UX} ${FTP_SFTP_USER}${FTP_SITE}:${NT_DIR}/${CATLG_NT}
    ...
    # rj132422 - byte count on the target via ssh (was the 'Information returned by' line from fileput.exp)
    FTPBYTECOUNT="$(ssh -nq ${FTP_SFTP_USER}${FTP_SITE} "wc -c ${NT_DIR}/${CATLG_NT}" | awk '{print $1}')"
    if [ "${GOODBYTECOUNT}" = "${FTPBYTECOUNT}" ]
    then
      echo "File transfer to ${NT_DIR} successful. ${CATLG_UX} count = ${GOODBYTECOUNT}"
    else
      echo "File transfer of ${CATLG_UX} to ${NT_DIR} failed: remote ${FTPBYTECOUNT}, real count is ${GOODBYTECOUNT}"
      $(abndalrt.ksh 911)
    fi
  else
...
#*****************************************************************************************
# END oem_ref_mptr299_ftp_tesla_source_file.ksh
#*****************************************************************************************
