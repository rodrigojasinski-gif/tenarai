#!/bin/ksh
echo "RCS $Id: oem_ref_mptr289_ftp_kaw_source_file.ksh,v 1.4 2016/11/02 23:59:18 pg2697 Exp $"
set -xv
#*****************************************************************************************
# PROCNAME oem_ref_mptr289_ftp_kaw_source_file.ksh                                              
# PURPOSE  Transfer the Kawasaki File from their External FTP Site
#          Unzip the file and build the required files (Price/Supersession and Substitution)
#          files as the Source Input Files for mptr290.
#          Additionally, move the zipped file to the Network and send an email notification.
#*****************************************************************************************
...
#STEP Step010R
  ...
  oem_job_status_update.ksh "R" "${STEPNAME} oem_ref_mptr289_ftp_kaw_source_file"
  #**************************************************************************************
...
#STEP Step020R
  ...
  then
    ...
    # rj132422 - prod3nt sunset: deliver to Mitchell oem_research via scp (was: fileput.exp ${BACKUP_SOURCE_FILE_NAME} to ${NOVELL}oem_research)
    ...
    FTPBYTECOUNT="$(ssh -nq ${FTP_SFTP_USER}${FTP_SITE} "wc -c < ${RESEARCH_DIR}/${BACKUP_SOURCE_FILE_NAME}")"  # rj132422 remote byte-count via ssh
    ...
    then
      ...
      echo "   3. You may delete the *.dat files"                                   >> ${MAIL_TEXT}
  ...
  ##########################################################################
   unzip -ojLL ${SOURCE_FILE} -d ${RACE}/tmp/${JOBNAME}
  ...
  # NOTE:
  # The Price file (iminf008.dat) MUST be found and transferred.
  # PRICE_NT must match the reformat job's (mptr290) INPUT_FILE_NAME.
  ##########################################################################
  ...
  then
    ...
    ########################################################################################
    # rj132422 - prod3nt sunset: scp to Mitchell outgoing (was: fileput.exp ${PRICE_UX} ${PRICE_NT} ${NT_DIR})
...
#STEP Step999R
  ...
  #             tmp/mptr289 directory. (NOTE: -r flag used with rm command.)
  ##########################################################################
  rm -rf ${RACE}/tmp/${JOBNAME}*
  

#*****************************************************************************************
# END oem_ref_mptr289_ftp_kaw_source_file.ksh
#*****************************************************************************************
