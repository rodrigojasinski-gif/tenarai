...
then
  ...
  # rj132422 - use last suffix, lowercased, so names with extra dots resolve correctly (was: cut -f2 -d".")
  export FILE_SUFFIX=$(echo ${INPUT_FILE_NAME##*.} | tr '[:upper:]' '[:lower:]')
  # rj132422 - prod3nt sunset: fileget.ksh unzipped these disguised-ZIP suffixes before delivery; restored here (was: zip/ZIP only)
  case ${FILE_SUFFIX} in
     zip|zzz|piz|zzp|zpp)
        echo "\nFILE_SUFFIX --> ${FILE_SUFFIX} : unzipping ${FTP_PULL_TARGET}\n"
