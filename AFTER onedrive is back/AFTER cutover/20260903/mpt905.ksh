...
#STEP Step020R
  ...
  # rj132422 - prod3nt sunset: the cdprod02/ftp_data share is now NFS-mounted at /<lvl>/ftp_data (was: ${NOVELL}oem)
  NT_DIR=/${ACT_LVL}/ftp_data/${ACT_LVL}/oem
  ...
  then
    ...
    ########################################################################################
    # rj132422 - prod3nt sunset: direct copy to the NFS-mounted share (was: fileput.exp ${REPORT_NT} to ${NT_DIR})
    cp ${REPORT_UX} ${NT_DIR}/${REPORT_NT} > ${FTP_LOGFILE} 2>&1

    ########################################################################################
    # Verify the copy is good
    ########################################################################################
    GOODBYTECOUNT="$(wc -c ${REPORT_UX} | awk '{print $1}')"
    FTPBYTECOUNT="$(wc -c ${NT_DIR}/${REPORT_NT} 2>/dev/null | awk '{print $1}')"  # rj132422 byte-count on the copied file (was: fileput 'Information returned by')
    ...
    then
      ...
      # rj132422 - space-separated for RHEL s-nail (was: ';' separator - s-nail rejects both ';' and ',')
      PROD_MAIL_RECIP="Rpt.OEM.Developers@Mitchell.com RACEDataOps@Mitchell.com"
      TEST_MAIL_RECIP="Rpt.OEM.Developers@Mitchell.com"
      ...
      # rj132422 - prod3nt sunset: point to the NFS-mounted share (was: prod3nt/cdprod02/ftp_data/<lvl>/oem)
      echo "   ${NT_DIR}/${REPORT_NT}" >> ${MAIL_TEXT}
