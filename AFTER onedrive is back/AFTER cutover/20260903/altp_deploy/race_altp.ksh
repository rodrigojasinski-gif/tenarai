...
then
   ...
   # rj132422 - final NAS location: pawsvm7001a:/cdprod02/ftp_data mounted at /prod/ftp_data
   # (was: /nas/prod/OEM_Repair_Doc_Repository/ftpdata/prod - temporary location while the share was pending)
   export ALTP_FTP_DATA=/prod/ftp_data/prod
else
   ...
   export MFConnect=''
   # rj132422 - final NAS location, dev side (confirm the RADD mount before deploying there)
   # (was: /nas/mdev/OEM_Repair_Doc_Repository/ftpdata/mdev - temporary location while the share was pending)
   export ALTP_FTP_DATA=/${ACT_LVL}/ftp_data/${ACT_LVL}
fi
...
# rj132422 - removed test-only line "export PATH=$HOME:$PATH": putting $HOME first lets a personal
# copy of any script shadow the deployed one in share/bin. Must not ship to production.
