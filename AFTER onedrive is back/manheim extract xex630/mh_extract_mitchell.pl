...
#$Id: mh_extract_mitchell.pl,v 1.7 2025/05/12 17:59:04 rd131153 Exp $   
print "\n" . "##################################################################################";
print "\n" . "#  PROCNAME:   mh_extract_mitchell.pl                                                     #";
...
## send as a bind variable the Oracle directory object shared/global env. variable
  ...
  my $sql_source = qq(
        	BEGIN
 	               :err_code := pkg_mitchell_extract_rj.sf_mitchell_extract(:pathout);
		END;
 	             );
  ...
  if ($err_code == -1)  { 
      $message = ("Error subr extract_files: sf_mitchell_extract_rj failed");
      return -1;        }
